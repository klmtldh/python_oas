...
class OasMeeting(object):
    ...
    def __init__(self,
        ...
        self.analysis_name_pptx = analysis_name + 'pptx'
        # 数据开始时间
    ...
    def write_pptx(self):
        ...
        # 建立PPTX模板
        op = oas_pptx.OasPptx(r'D:\JGY\600-Data\004-auxiliary辅助文件\昆明供电局早会通报安监部模板.pptx')
        # 模板首页文字字符串
        first_text_list = [
            '',
            self.now_day.get_chinese_date()[0]+self.analysis_name,
            '安全监管部（应急指挥中心）',
            self.now_day.get_chinese_date()[3],
            '昆明供电局2021年生产调度早会通报材料'
        ]
        text_list = []
        for i, slide in enumerate(op.prs.slides):
            for j, shape in enumerate(slide.shapes):
                if not shape.has_text_frame:
                    continue
                text_frame = shape.text_frame
                for k, paragraph in enumerate(text_frame.paragraphs):
                    p = text_frame.paragraphs[k]
                    text_list.append('第'+str(i)+'页'+'第'+str(j)+'shape'+'第'+str(k)+'段：'+p.text)
        doc = oas_docx.OasDocx()
        for item in text_list:
            doc.par(item)
        doc.save_docx(r'D:\JGY\600-Data\006-temporary临时文件\ppt.docx')
...
def main():
    analysis_category = '会议'
    analysis_name = '昆明供电局安全情况通报'
    meeting = OasMeeting(analysis_category, analysis_name)
    meeting.write_pptx()

...
def sfb_plan_save_path(file_type):
    ...
    m_name = '问题库'
    if file_type == 'docx':
        f_name = '昆明供电局安监部立行立改问题库执行情况docx'
    elif file_type == 'pptx':
        f_name = '昆明供电局安监部立行立改问题库执行情况pptx'
    io_name = '输出'
...
def pandas_rat_text(df_list):
    ...
    for i, item in enumerate(df_list):
        all_text = '昆明供电局安全监管部（应急指挥中心）立行立改问题库共有{0}条问题{13}，截至{1}，完成{2}条，未完成{3}条，完成率{4};'\
    '其中：安监科共有{5}条计划，完成{6}条，未完成{7}条，完成率{8}。'\
    '应急科共有{9}条计划，完成{10}条，未完成{11}条，完成率{12}。'.format(item.loc[item['责任科室'] == '安监部', '合计'].values[0],
...
def write_docx(sql):
    now_day = oas_n_day.NDay(datetime.date.today())
    doc = OasDocx()
    doc.hd('昆明供电局安全监管部（应急指挥中心）立行立改问题库执行情况', font_size=22)
    ...
    h_columns = ['发现问题', '行动计划',  '开始时间', '完成时间', '是否完成', '责任科室', '责任人员', '配合人员', '目前推进情况']
    ...
    for name in duty_name:
        ...
        doc.par('（{0}）{1}'.format(digital_to_chinese(j), name))
        if analysis_name_list == []:
            doc.par('无异常')
        else:
            i = 1
            for item in analysis_name_list:
                doc.par('{0}.未完成事项{1}'.format(i, digital_to_chinese(i)))
                x = item.split('\n')
                for y in x:
                    doc.par(y)
                i += 1
...
def write_pptx(sql):
    ...
    text_list = ['昆明供电局安全监管部（应急指挥中心）立行立改问题库执行情况',
                 '安全监管部（应急指挥中心）',
                 now_day.get_chinese_date()[3]]
    ...
    op.page()
    op.text('执行中问题科室完成情况表')
    op.p_table(df_table[1])
    op.page()
    op.text('执行中问题个人完成情况表')
    ...
    s_columns = ['开始时间', '完成时间', '是否完成', '责任人员']
    op.text('正在执行中个人问题明细')
    duty_name_list = ['（'+digital_to_chinese(i+1)+'）'+item for i, item in enumerate(duty_name)]
    ...
    for name in duty_name:
        ...
        if analysis_name_list == []:
            op.text_n('（'+digital_to_chinese(j)+'）'+name)
...
# 计划分析模块
def sfb_promptly_problem_main():
    # 用Excel文件更新MySQL数据库
    sfb_plan_update()
    # 将MySQL中的表备份到本地Excel
    sfb_plan_backup()
    # 统一查询库
    sql = "select * from 昆明供电局安监部立行立改问题库"
    # Word文档自动生成
    write_docx(sql)
    # PPT文档自动生成
    write_pptx(sql)
